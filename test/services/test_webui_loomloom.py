...
from app.services import loomloom, shengsuanyun_video
...
def test_shengsuanyun_video_source_passes_secret_only_in_confirmed_request():
    test_config = dict(
        config.app,
        script_generation_backend="loomloom",
        loomloom_base_url="https://example.test/loom/v1",
        ...
        shengsuanyun_video_base_url="https://example.test/api/v1",
        shengsuanyun_video_model=shengsuanyun_video.DEFAULT_MODEL_ID,
    )
    ...
    with (
        ...
        assert (
            _widget_by_key(app.number_input, "shengsuanyun_video_scene_count").value
            == 1
        )
        assert (
            _widget_by_key(app.selectbox, "shengsuanyun_video_model_select_en").value
            == shengsuanyun_video.DEFAULT_MODEL_ID
        )

        _widget_by_key(app.checkbox, "shengsuanyun_video_confirm_charge").check().run()
        ...
        video_request = submit_generation.call_args.kwargs["shengsuanyun_video_request"]
        ...
        assert video_request.batch.model_id == shengsuanyun_video.DEFAULT_MODEL_ID
        # 直连 POST 没有幂等键，提交后必须记录本地任务并阻止重复付费请求。
        assert app.session_state["shengsuanyun_video_submitted_task_id"]
        assert [str(item.value) for item in app.exception] == []


def test_completed_shengsuanyun_task_resets_charge_confirmation_safely():
    """付费任务结束后的整页 rerun 必须清理确认框且不能触发状态异常。"""
    task_id = "2fbf62f1-47f3-43c4-96d5-67e149baaad5"
    state = MemoryState()
    state.update_task(
        task_id,
    ...
    test_ui = dict(config.ui, open_task_folder_on_completion=False)
    ...
    with (
        ...
        app = AppTest.from_file(str(WEBUI_MAIN), default_timeout=30)
        app.session_state["ui_language"] = "en"
        app.session_state["current_generation_task_id"] = task_id
        app.session_state["active_generation_tasks"] = {
            task_id: {"subject": "AI daily life"}
        }
        app.session_state["shengsuanyun_video_submitted_task_id"] = task_id
        app.session_state["shengsuanyun_video_confirm_charge"] = True
        app.run()

        assert not app.session_state["shengsuanyun_video_confirm_charge"]
        assert app.session_state["shengsuanyun_video_submitted_task_id"] == ""
        assert [str(item.value) for item in app.exception] == []
