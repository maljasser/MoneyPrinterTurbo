...
class TestTaskService(unittest.TestCase):
    ...
    def test_shengsuanyun_material_failure_keeps_remote_request_id(self):
        """远端运行已创建后失败，任务状态必须保留请求 ID 和所选模型。"""
        params = VideoParams(video_subject="AI 办公", video_source="loomloom")
        settings = tm.shengsuanyun_video.ShengsuanVideoSettings(
            base_url="https://example.test/api/v1",
            api_token="test-token",
        )
        batch = tm.shengsuanyun_video.ShengsuanVideoBatch(
            model_id=tm.shengsuanyun_video.DEFAULT_MODEL_ID,
            prompts=("office worker",),
            aspect_ratio="9:16",
        )
        request = tm.shengsuanyun_video.ShengsuanConfirmedVideoRequest(
            settings=settings,
            batch=batch,
        )
        backend = MagicMock()

        def fail_after_submit(batch, destination, on_request_submitted):
            on_request_submitted("request-1")
            raise tm.shengsuanyun_video.ShengsuanVideoRunError("remote run timeout")

        backend.generate_and_download.side_effect = fail_after_submit
        state = MemoryState()
        state.update_task(
        ...
        with (
            patch.object(tm.sm, "state", state),
            patch.object(
                tm.shengsuanyun_video,
                "ShengsuanVideoBackend",
            ...
            result = tm.get_video_materials(
                ...
                ["office worker"],
                10,
                shengsuanyun_video_request=request,
            )
        ...
        self.assertEqual(failed_task["shengsuanyun_request_ids"], ["request-1"])
        self.assertEqual(
            failed_task["shengsuanyun_video_model"],
            tm.shengsuanyun_video.DEFAULT_MODEL_ID,
        )
